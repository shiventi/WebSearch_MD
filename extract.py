...
client = MongoClient(st.secrets["mongodb"]["user"], serverSelectionTimeoutMS=15000, maxPoolSize=10)
db = client.dataset
question_collection = db.chats
...
def web_search(url, query, element_type=None, class_name=None):
    ...
    cleaned_results = [result.get_text() for result in search_results if result.find('b')]
    return cleaned_results

def open_file(u_type):
    x = question_collection.find()
    if u_type.lower() == "question":
        return [i["kv"].split("  ")[0] for i in x]
    elif u_type.lower() == "answer":
        return [i["kv"].split("  ")[1] for i in x]
...
def write_data(user_input, q_a=None):
    ...
    if max(similarities) > 0.7:
        ...
        lister = [str(document["kv"]) for document in cursor]
        ...
        if float(c[1]) > 0.8:
            ...
            question_collection.insert_one(new_question)
...
def main():
    ...
    st.warning("This app runs slowly because it fetches data from the web", icon="⚠️")
